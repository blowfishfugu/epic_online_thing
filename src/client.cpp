...
void update_game(s_platform_data platform_data, s_platform_funcs platform_funcs)
{
	...
	g_gamewindow.size = v2ii(g_gamewindow.width, g_gamewindow.height);
	g_gamewindow.center = v2_mul(g_gamewindow.size, 0.5f);
...
void render(float dt)
{
	switch(state)
	{
		...
		case e_state_game:
		{
			...
			// @Note(tkap, 23/06/2023): Display how many seconds left to beat the level
			{
				...
				s_v2 pos = v2(
					g_gamewindow.center.x,
					g_gamewindow.size.y * 0.3f
				);
	...
	draw_rect(g_gamewindow.center, 0, g_gamewindow.size, v41f(1), {.do_background = true});

	{
		...
		glViewport(0, 0, g_gamewindow.width, g_gamewindow.height);
		// glEnable(GL_DEPTH_TEST);
		glDepthFunc(GL_GREATER);

		{
			int location = glGetUniformLocation(g_program, "window_size");
			glUniform2fv(location, 1, &g_gamewindow.size.x);
		}
		{
